...
import { CollectionManager, type HTMLPayload } from '@/managers/CollectionManagerTs';
import { MasonryTs, Virtualizer } from '@/ui';
...
onMount(async () => {
  ...
  if (!disableSet.has('virtualisation')
    && result.length >= 60
  ...
  if (!disableSet.has('masonry')) {
    const masonryManager = new MasonryTs(collectionManager.collection)
