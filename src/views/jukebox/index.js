...
/**
 * Render a grid of music from your vault. Files can have an audio embedded, an url or both
  ...
  */
  ...
  async function renderView({ vm, logger }) {
    ...
    if (!vm.disableSet.has("buttons")) {
      ...
      const youTubeManager = new YouTubeManager({ logger })
    ...
    const pageToChild = async (p) => {
      ...
      const ytVideo = YouTubeManager?.extractInfoFromYouTubeUrl(p[URL_FIELD])

      if (!vm.disableSet.has("thumbnail")) {
        if (!isValidPropertyValue(p[THUMBNAIL_FIELD])) {
          if (ytVideo) {
            imgTag = renderer.renderImageFromUrl(YouTubeManager?.buildYouTubeImgUrlFromId(ytVideo.id))
          } else {
      ...
      if (p[URL_FIELD]) {
        let url = (FORCE_CLASSIC_YOUTUBE_URL && ytVideo) ? YouTubeManager?.buildYouTubeUrlFromId(ytVideo.id) : p[URL_FIELD]
...
<article class="item" ${articleStyle}>
    ...
    if (VIRTUALIZED_LAYOUT &&
      ...
      const virtualizedGridManager = new VirtualizedGrid({
        root: vm.content,
        manager: gridManager,
        // logger,
      })
    ...
    const gridLogger = new Logger({ app })

    await gridManager.bakeNextHTMLBatch()
    gridLogger?.logPerf("Baking first batch of cells")

    await gridManager.insertNewChunk()
    gridLogger?.logPerf("Inserting first cells in the DOM")
    ...
    if (MASONRY_LAYOUT && !vm.disableSet.has('masonry')) {
      ...
      const masonryManager = new Masonry(gridManager.parent)
