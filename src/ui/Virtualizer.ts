import type { CollectionManager } from "@/managers/CollectionManagerTs";
import type { Logger } from "@/modules/LoggerTs";
import { closestTo, createFragmentFromString, debounce } from "@/modules/utils";

interface BakedItem {
  /** The actual HTML representation of the item */
  html: string;

  /** Additional properties or data for the item */
  extra: object;

  /** The same HTML representation of the item patched with dynamic hardcoded style */
  patchedHtml: string;
}

interface IntersectedAnchors {
  top: number;
  bottom: number;
}

interface MonitoredBatch {
  /** The batch ID */
  id: number;

  /** List of baked items in this batch */
  bakedItems: BakedItem[];

  /**
   * Indicates whether the items in the batch have been patched.
   * Defaults to false. Set to true when virtualized.
   */
  patched: boolean;

  /**
   * Indicates whether the batch is currently loaded in the DOM.
   * Used to prevent multiple stamping of the same batch.
   */
  withinTheDOM: boolean;

  /**
   * Number of anchors intersected with the viewport (both top and bottom).
   * Represents how many anchors are not visible in the viewport.
   */
  intersectedAnchors: IntersectedAnchors;
}

interface VirtualizedCollection {
  /** The width of the grid */
  gridWidth: number;

  /** A collection of monitored batches, where the key is the batch ID */
  monitoredBatches: Record<number, MonitoredBatch>;
}
 ...
 *
 * - The *virtualisation* of an item is the action of removing it from the DOM while keeping its relative position and size in memory.
 *
 * - The *stamping* of an item is the action of putting it back in the DOM using the position and size previously stored in memory.
 *
 * ***Why is it needed?***
 *
 ...
 *
 * Thanks to this attribute and the location of the intersection, we can decide both the action to be taken and the batch to be targeted
 *
 ...
 *
 * - Based on the way it's implemented, it doesn't work in .canvas files
 *
 * Indeed, the `--zoom-multiplier` css variable used to determine the zoom applied on the canvas doesn't correspond to actual size difference of the element visible on it when doing the computation.
 * Also there aren't any logical correlation I can deduce from it unfortunatly. I could theoritically rely on another metric in this case (like the horizontal length of one cell) but I prefer not to for now
 *
 * - It only works when batches are of a consistent size
 *
...
export class Virtualizer<T> {
  ...
  virtualizedCollection: VirtualizedCollection;
  stubs: NodeListOf<HTMLElement> | null;
  settings: { rowGap: number; columnGap: number; itemImageHeight: number };
  root: HTMLElement;
  ...
  constructor({
    ...
    logger,
    root,
    numberOfItemPerBatch = NaN,
  }: {
    manager: CollectionManager<T>;
    logger?: Logger;
    root: HTMLElement | null;
    numberOfItemPerBatch?: number;
  }) {
    if (!manager)
      throw new Error(
        "Can't create a Virtualized layout without a valid manager of the said layout"
      );

    this.manager = manager;
    this.logger = logger;

    this.grid = manager.collection;
    ...
    const [columnGap, rowGap] = [
      parseInt(gridComputedStyle.getPropertyValue("column-gap")),
      parseInt(gridComputedStyle.getPropertyValue("row-gap")),
    ];
    this.settings = {
      rowGap: !Number.isNaN(rowGap) ? rowGap : 0,
      columnGap: !Number.isNaN(columnGap) ? columnGap : 0,
      itemImageHeight: parseInt(
        gridComputedStyle.getPropertyValue("--jukebox-cards-image-height")
      ),
    };
    ...
    /**
     * @todo Using this might handle the unconsistent arrival of element in the DOM
     */
    this.numberOfItemPerBatch = numberOfItemPerBatch;
  ...
  computeNumberOfColumns() {
    ...
    for (let numberOfColumns = 1; ; numberOfColumns++) {
      const estimatedWidth =
        this.settings.itemWidth * numberOfColumns +
        this.settings.columnGap * (numberOfColumns - 1);
  ...
  computeItemColumnId(item) {
    return Math.round(
      (item.offsetLeft + this.settings.columnGap) /
        (this.settings.itemWidth + this.settings.columnGap)
    );
  }
  ...
  #computeRelevantInformationForOneItemIntersection(entry, context) {
    ...
    const fromTop =
      closestTo(rootBounds.top, rootBounds.bottom, boundingClientRect.top) ===
      rootBounds.top;
    ...
    if (context === "stamping") {
      triggerWhenReaching ??= batchPart === "start" ? "top" : "bottom";
    } else if (context === "virtualisation") {
      triggerWhenReaching ??= batchPart === "start" ? "bottom" : "top";
    }

    // It's a false positive
    if (
      (triggerWhenReaching === "bottom" && fromTop) ||
  ...
  #handleItemIntersectionForStamping(entries) {
    ...
    entries.forEach(async (entry) => {
      ...
      const { fromTop, targettedBatchId } =
        this.#computeRelevantInformationForOneItemIntersection(
          entry,
          "stamping"
        ) || {};
      ...
      if (
        this.#tryToStampBatchOfId(targettedBatchId, fromTop ? "begin" : "end")
      ) {
        this.#logStatus();
      }
    });
  ...
  #tryToStampBatchOfId(batchId, location) {
    ...
    if (
      this.virtualizedCollection.monitoredBatches[batchId].withinTheDOM ||
      this.grid.querySelectorAll(
        `${this.manager.childTag}[data-batch="${batchId}"]`
      ).length > 0
    ) {
    ...
    console.warn(
      `Make some noise! Batch ${batchId} is making it's great return into the DOM!`
    );
    this.virtualizedCollection.monitoredBatches[batchId].withinTheDOM = true;
    const computedBatchToStamp = this.virtualizedCollection.monitoredBatches[
      batchId
    ].bakedItems.map((item) => {
      return {
    ...
    if (
      location === "end" &&
      !this.virtualizedCollection.monitoredBatches[batchId + 1]?.patched
    ) {
  ...
  #handleItemIntersectionForVirtualisation(entries) {
    ...
    entries.forEach(async (entry) => {
      ...
      const { batchId, targettedBatchId, fromTop } =
        this.#computeRelevantInformationForOneItemIntersection(
          entry,
          "virtualisation"
        ) || {};
      ...
      const targettedBatch =
        this.virtualizedCollection.monitoredBatches[targettedBatchId];
      if (!targettedBatch || targettedBatch.withinTheDOM === false) {
        // console.warn(`Failed to virtualize - Batch ${targettedBatchId} is not in the DOM!`)
        return;
      }
      ...
      const batchItemsToVirtualize = [
        ...this.grid.querySelectorAll(
          `${this.manager.childTag}[data-batch="${targettedBatchId}"]`
        ),
      ];
      if (batchItemsToVirtualize.length === 0) {
        // console.warn(`Failed to virtualize - There aren't any items from batch ${targettedBatchId} within the DOM!`)
      ...
      if (fromTop) {
        ...
        this.logger?.warn(
          `Batch ${batchId} will soon be off the screen, let's virtualize the batch above it (${targettedBatchId})!`
        );
        ...
        this.logger?.warn(
          `Batch ${batchId} will soon be off the screen, let's remove the batch below it (${targettedBatchId})!`
        );
        if (
          this.virtualizedCollection.monitoredBatches[targettedBatchId + 1]
            ?.withinTheDOM
        ) {
          /**
           * This is a HUGE hack.
           ...
           */
          this.logger?.error(
            "What are you doing!? The batch below the targetted batch is still in the DOM!"
          );
          return;
        }
  ...
  #removeBatchOfIdFromDOM(batchId) {
    const nodesToDelete = this.grid.querySelectorAll(
      `${this.manager.childTag}[data-batch="${batchId}"]`
    );
  ...
  #computeMutationRecordBatchIndex = (mutationRecord) => {
    ...
    if (
      ...
      !mutationRecord.nextSibling?.dataset?.batch
    ) {
      return 0;
    }
    ...
    if (mutationRecord.previousSibling.classList.contains("stub")) {
      const nextSibling =
        mutationRecord.addedNodes[mutationRecord.addedNodes.length - 1]
          .nextSibling;
      if (!nextSibling) {
        return 0;
      }
    ...
    const anchorBatchIndexAttribute =
      mutationRecord.previousSibling.dataset.batch;
    ...
    for (const mutation of mutationRecord) {
      if (mutation.type === "childList") {
        ...
        if (addedNodes.length < numberOfItemsToObserve) {
          console.warn(
            `There aren't enough items per batch to apply the collection virtualisation (The grid might be too big / the screen too large)`
          );
          this.logger?.warn(
            `There aren't enough items per batch to apply the collection virtualisation (The grid might be too big / the screen too large)`
          );
        ...
        const insertedBatchIndex =
          this.#computeMutationRecordBatchIndex(mutation);

        console.log(
          `VG - Current inserted batch index is ${insertedBatchIndex}`
        );
        ...
        this.virtualizedCollection.monitoredBatches[
          insertedBatchIndex
        ].withinTheDOM = true;
   ...
   */
  #setupNodesDatasetAndObserveItemsOfBatch({
    nodes,
    batchIndex,
    numberOfItemsToObserve,
  }) {
    ...
    for (
      ...
      i > nodes.length - numberOfItemsToObserve - 1;
      i--
    ) {
  ...
  #handleGridResize(entries) {
    ...
    if (this.virtualizedCollection.gridWidth !== currentEntryWidth) {
      this.logger?.warn(
        `The grid width changed from ${this.virtualizedCollection.gridWidth} to ${currentEntryWidth}. Farewell sweet optimization!`
      );
      this.virtualizedCollection.gridWidth = currentEntryWidth;
  ...
  #initObservers(root = null) {
    ...
    const intersectionOptions = {
      root, // if `null`, it is relative to the viewports
      threshold: 0, // how much of the target must be visible. Here it means as soon/long as there is one pixel visible
    };

    this.itemIntersectionObserverForVirtualisation = new IntersectionObserver(
      this.#handleItemIntersectionForVirtualisation.bind(this),
      intersectionOptions
    );
    this.itemIntersectionObserverForStamping = new IntersectionObserver(
      this.#handleItemIntersectionForStamping.bind(this),
      intersectionOptions
    );

    this.gridMutationObserver = new MutationObserver(
      this.#handleGridMutation.bind(this)
    );
  ...
  reset() {
    ...
    if (!this.virtualizedCollection.monitoredBatches[0].withinTheDOM) {
      ...
      for (
        let i =
          Object.keys(this.virtualizedCollection.monitoredBatches).length - 1;
        i >= 0;
        i--
      ) {
      ...
      requestAnimationFrame(() => {
        this.init();
        Object.entries(this.virtualizedCollection.monitoredBatches).forEach(
          ([batchId, batch]) => {
            const nodesToUpdate = this.grid.querySelectorAll(
              `.item[data-batch="${batchId}"]`
            );
   ...
   * @param {Element[]} _.items
   */
  #reallyPrepareVirtualisationOfItemsFromMonitoredBatch__steady = ({
    serializer,
    bakedItems,
    newStubsHeight,
    columnIds,
    ...
    items.forEach((item, i) => {
      ...
      const newX =
        columnId * itemRect.width + columnId * this.settings.columnGap;
...
${currentStyle ?? ""}`;
  ...
  #prepareVirtualisationOfItemsFromMonitoredBatch({ monitoredBatch, items }) {
    ...
    const newStubsHeight = Array.from(
      this.stubs,
      (stub) => stub.getBoundingClientRect().height
    );
    ...
    this.logger?.logPerf(
      `Finished preparing virtualisation of ${items.length} items from batch ${monitoredBatch.batchId}`
    );
