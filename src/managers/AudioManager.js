import { clamp, convertTimecodeToDuration, getOS } from '@/modules/utils'
...
export class AudioManager {
    ...
    onPlayAudio = async ({ index, audios, playButtons }) => {
        ...
        if (!isNaN(dataVolume)) {
            audios[index].volume = clamp(this.defaultVolume + dataVolume, 0.1, 1)
        } else {
            audios[index].volume = clamp(this.defaultVolume, 0.1, 1)
        }
        ...
        playButtons[index].dataset.state = 'playing'
    }
