...
export class CollectionManager {
  ...
  async bakeNextHTMLBatch() {
    ...
    // Map pages to promises and run them concurrently
    const HTMLBatchPromises = pagesBatch.map(async (page) => {
    ...
    const HTMLBatch = (await Promise.all(HTMLBatchPromises)).flat();
