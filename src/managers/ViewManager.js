...
export class ViewManager {
   ...
   */
  constructor({ disable = "", app, component, container, logger, name, debug = false } = {}) {
    this.app = app
    ...
    //getParentWithClass(this.host.parentNode, "view-content")
   ...
   */
  #resolveCurrentLeaf() {
    let leaf = getParentWithClass(this.host.parentNode, "workspace-leaf")
    if (leaf) {
      this.logger?.log("We've found a leaf, and it looks like this view is in a classic tab 🗨️")
      return leaf
    }

    leaf = getParentWithClass(this.host.parentNode, "popover")
    if (leaf) {
  ...
  #resolveCurrentContent() {
    let content = getParentWithClass(this.host.parentNode, "markdown-embed-content")
    if (content) {
      this.logger?.log("We're in a specific type of file")
      return content
    }

    content = getParentWithClass(this.host.parentNode, "kanban-plugin")
    if (content) {
      this.logger?.log("We're in a Kanban card")
      if (content.firstChild?.classList?.contains("kanban-plugin__horizontal")) {
        content = getParentWithClass(this.host.parentNode, "kanban-plugin__lane-items")
      }
      return content
    }

    content = getParentWithClass(this.host.parentNode, "view-content")
    if (content) {
  ...
  #amiInPopover() {
    if (!!getParentWithClass(this.host.parentNode, "popover")) return true
  ...
  #amiInCallout() {
    return !!getParentWithClass(this.host.parentNode, "callout-content")
  }
  ...
  */
  ...
  #amiInEmbed() {
    return !!getParentWithClass(this.host.parentNode, "markdown-embed-content")
  }
  ...
  inWhichFiletypeAmi() {
    if (getParentWithClass(this.host.parentNode, "canvas-node-content")) {
      return 'canvas'
    }
    if (getParentWithClass(this.host.parentNode, "kanban-plugin")) {
      return 'kanban'
    }
    return 'normal'
  }
  ...
  #computeClassName = (debug) => {
    let className = "custom-view custom-view-full-width " + this.name
