...
export class PageManager {
  ...
  constructor({
     ...
     */
    this.invertedUserFields = buildInvertedMap(this.userFields)
    ...
    this.querySortFunctionsMap.set("manual", async (pages, field) => {
      ...
      const sortingPages = await normalizeLinksPath(
        rawSortingPages,
  ...
  buildAndRunFileQuery = async ({ filter, qs, initialSubset }) => {
    ...
    if (typeof filter === "function") {
      ...
      this.#runStringFilterQuery({ filter, qs, keepCurrentPages: !isEmpty(initialSubset) })
    } else if (Array.isArray(filter)) {
      this.#runArrayFilterQuery({ filter, qs, keepCurrentPages: !isEmpty(initialSubset) })
    } else if (filter instanceof RegExp) {
      this.#runStringFilterQuery({ filter, qs, keepCurrentPages: !isEmpty(initialSubset) })
    } else {
      await this.#runObjectFilterQuery({ filter, qs, keepCurrentPages: !isEmpty(initialSubset) })
    }
  ...
  #sortPages = async ({ sort, pages, options }) => {
    ...
    if (sort?.manual) {
      ...
      const sortingPages = await normalizeLinksPath(rawSortingPages, this.orphanage.directory)
    ...
    if (sort?.shuffle) {
      if (typeof sort.shuffle === "boolean") {
        ...
        return shuffleArray(pages, this.seed);
      } else if (typeof sort.shuffle === "number") {
        this.queryDefaultSortFunctionsMap.get("path")(pages)
        return shuffleArray(pages, sort.shuffle);
      }
    }
